...
backend_args = None
data_root = 'data/zx_split_ir/'
dataset_type = 'PeopleDataset'
default_hooks = dict(
...
model = dict(
    ...
    decoder=dict(
        layer_cfg=dict(
            ...
            ffn_cfg=dict(
                embed_dims=256, feedforward_channels=512, ffn_drop=0.1),
            self_attn_cfg=dict(
    ...
    teacher_model_load_from='./teacher.pth',
    test_cfg=dict(max_per_img=100),
    train_cfg=dict(
        assigner=dict(
            match_costs=[
...
test_evaluator = dict(
    ann_file='data/zx_split_ir/annotations/instances_val2017.json',
    backend_args=None,
    format_only=False,
    metric='bbox',
    type='CocoMetric')
